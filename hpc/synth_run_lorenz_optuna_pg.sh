#!/bin/bash
#SBATCH --output=/cluster/home/ja1659/logs/lorenz_batch.out
#SBATCH --error=/cluster/home/ja1659/logs/lorenz_batch.err
...
# Load JSON Config
dataset_config=${code_path}"/configs/lorenz.json"
...
for job_id in $(seq 1 $num_jobs); do
    job_file="${tmp_folder}/tmp_job_lorenz_${job_id}.sh"
    cat <<EOL > ${job_file}
#!/bin/bash
#SBATCH --job-name=optuna_lorenz_${job_id}
#SBATCH --output=/cluster/home/ja1659/logs/optuna_lorenz_${job_id}.out
#SBATCH --error=/cluster/home/ja1659/logs/optuna_lorenz_${job_id}.err
#SBATCH --ntasks=1
...
singularity exec --bind ${datapath}:/usr/data --bind ${code_path}:/usr/src ${singularity_image} \
    /bin/bash -c "cd /usr/src && python3 -m synthetic_data.lorenz_model --num_trials  ${trials_per_job} --num_jobs ${num_cpus} --run_best False --is_optuna True ${config_params}"
EOL
