#!/bin/bash
#SBATCH --output=/cluster/home/ja1659/logs/multiplex_acdc_batch.out
#SBATCH --error=/cluster/home/ja1659/logs/multiplex_acdc_batch.err
...
dataset_config=${code_path}"/configs/cardiac_acdc.json"  # Anat
# dataset_config=${code_path}"/configs/cardiac_acdc_all.json"  # Full
...
for job_id in $(seq 1 $num_jobs); do
    ...
    cat <<EOL > ${job_file}
#!/bin/bash
#SBATCH --job-name=optuna_acdc_${job_id}
#SBATCH --output=/cluster/home/ja1659/logs/optuna_multiplex_acdc_${job_id}.out
#SBATCH --error=/cluster/home/ja1659/logs/optuna_multiplex_acdc_${job_id}.err
#SBATCH --ntasks=1
...
singularity exec --bind ${datapath}:/usr/data --bind ${code_path}:/usr/src ${singularity_image} \
    /bin/bash -c "cd /usr/src && python3 -m experiments.ACDC_CV --num_trials ${trials_per_job} --num_jobs ${num_cpus} ${config_params}"
EOL
